    ...
    try:
        ...
        if LEDController:
            try:
                ...
                test_controller.turn_off_all()
                test_controller.turn_on_led(0, (255, 0, 0), auto_show=False)  # Red test
...
def test_individual_led():
    ...
    try:
        ...
        try:
            ...
            controller.turn_on_led(led_index, tuple(color), brightness=brightness)
...
def _run_rainbow_sequence(controller, led_count, duration, speed, brightness, test_id):
    ...
    while time.time() - start_time < duration:
        ...
        for i in range(led_count):
            ...
            controller.turn_on_led(i, color, brightness=brightness, auto_show=False)
...
def _run_chase_sequence(controller, led_count, duration, speed, brightness, colors, test_id):
    ...
    while time.time() - start_time < duration:
        ...
        for i in range(chase_length):
            ...
            controller.turn_on_led(led_pos, tuple(color), brightness=fade_brightness, auto_show=False)
...
def _run_fade_sequence(controller, led_count, duration, speed, brightness, colors, test_id):
    ...
    while time.time() - start_time < duration:
        ...
        for i in range(led_count):
            controller.turn_on_led(i, tuple(color), brightness=current_brightness, auto_show=False)
    ...
    while time.time() - start_time < duration:
        ...
        for i in range(led_count):
            ...
            if key_position in white_key_pattern:
                ...
                controller.turn_on_led(i, color, brightness=brightness * 0.8, auto_show=False)
            else:
                # Black key
                color = (64, 64, 64)  # Dark gray for black keys
                controller.turn_on_led(i, color, brightness=brightness * 0.3, auto_show=False)
        ...
        for step_index, step in enumerate(pattern):
            ...
            for led_index in leds:
                if 0 <= led_index < led_count:
                    controller.turn_on_led(led_index, tuple(color), brightness=step_brightness, auto_show=False)
