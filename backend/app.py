...
def play():
    ...
    try:
        ...
        # Then start playback
        success = playback_service.start_playback()
        if success:
            return jsonify({
                'status': 'success',
...
def trigger_startup_animation():
    ...
    try:
        if not led_effects_manager:
            return jsonify({
                'success': False,
                'message': 'LED effects manager not available'
            }), 503
        ...
        logger.error(f"Error starting LED startup animation: {e}")
        return jsonify({
            'success': False,
...
def _rainbow_sequence(led_count):
    ...
    for i in range(min(led_count, 246)):
        ...
        led_controller.turn_on_led(i, (int(r * 255), int(g * 255), int(b * 255)), auto_show=False)
    led_controller.show()
...
def _chase_sequence(led_count):
    ...
    for i in range(5):  # 5-LED chase
        ...
        led_controller.turn_on_led(led_pos, (int(255 * brightness), int(100 * brightness), 0), auto_show=False)
    led_controller.show()
...
def _piano_keys_sequence(led_count):
    ...
    for i in range(min(led_count, 246)):
        ...
        if key_pos in white_key_pattern:
            led_controller.turn_on_led(i, (255, 255, 255), auto_show=False)  # White
        else:
            led_controller.turn_on_led(i, (50, 50, 50), auto_show=False)     # Dark gray for black keys
    led_controller.show()
...
def handle_test_pattern(data):
    ...
    try:
        if not led_controller or not led_effects_manager:
            emit('error', {'message': 'LED controller not available'})
            return
        ...
        if pattern == 'rainbow':
            ...
            # Use the effects manager for threaded patterns
            led_effects_manager.start_effect(pattern, base_color)
    ...
    try:
        ...
        if LEDController:
            try:
                if led_controller:
                    ...
                    led_controller.turn_off_all() 
                led_controller = LEDController(num_pixels=LED_COUNT, settings_service=settings_service)
        ...
        if led_controller:
            try:
                ...
                for i in range(LED_COUNT):
                    led_controller.turn_on_led(i, color, brightness=brightness, auto_show=False)
                    # Small delay for visual effect
                    socketio.sleep(0.01)
                # Show once after the loop
