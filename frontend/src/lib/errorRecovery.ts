...
export class ErrorRecoveryManager {
	...
	processError(
		...
		errorAnalytics.trackError({
			...
			context: {
				...
				fileType: errorContext.fileInfo?.type,
				fileSize: errorContext.fileInfo?.size
			}
		});
	...
	async attemptRetry(
		...
		try {
			...
			errorAnalytics.trackRecovery(errorId, {
				method: 'retry',
				successful: true,
	...
	private categorizeError(error: any, context?: Partial<ErrorContext>): ErrorContext {
		...
		// Enhanced validation error detection
			...
			errorType = 'validation';
			severity = context?.fileInfo?.name ? 'medium' : 'low';
		...
		// Enhanced upload error detection
			...
			errorType = 'upload';
			severity = 'high';
			userMessage = context?.fileInfo?.name ? `Failed to upload ${context.fileInfo.name}` : 'Upload failed';
			technicalMessage = error.stack || '';
	...
	if (context?.fileInfo || context?.fileInfo?.name || errorType === 'file') {
		return 'file-upload';
	}
