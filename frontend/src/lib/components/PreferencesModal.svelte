...
<div class="modal-backdrop" on:click={handleCancel} role="button" tabindex="0" on:keydown={(e) => { if (e.key === 'Escape' || e.key === 'Enter' || e.key === ' ') { e.preventDefault(); handleCancel(); } }}>
	<div class="modal-content" role="dialog" aria-modal="true" aria-labelledby="preferences-title" tabindex="0" on:keydown={(e) => { if (e.key === 'Escape') { e.preventDefault(); handleCancel(); } }} on:click|stopPropagation>
