<script lang="ts">
	...
	// Backend socket connection status
...
<div class="network-midi-config">
	<div class="socket-status-badge {wsConnected ? 'connected' : 'disconnected'}">
		<span class="dot"></span>
		<span>{wsConnected ? 'Backend Socket Connected' : 'Backend Socket Disconnected'}</span>
	</div>
	<div class="header">
...
<style>
	...
	.socket-status-badge.connected {
		background: #d4edda;
		color: #155724;
	}
	.socket-status-badge.disconnected {
		background: #f8d7da;
		color: #721c24;
	}
	.socket-status-badge .dot {
		width: 8px;
		height: 8px;
		border-radius: 50%;
