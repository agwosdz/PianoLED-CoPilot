...
global.File = class MockFile implements File {
	...
	constructor(bits: (string | ArrayBuffer | ArrayBufferView)[], name: string, options: MockFileOptions = {}) {
		this.bits = bits;
		this.name = name;
		// Calculate size properly for string content
		this.size = bits.reduce((acc: number, bit: string | ArrayBuffer | ArrayBufferView) => {
			if (typeof bit === 'string') {
				return acc + bit.length;
			}
			return acc + ((bit as any).length || (bit as ArrayBuffer).byteLength || 0);
		}, 0);
		...
		this.text = async () => this.bits.join('');
		this.slice = (start?: number, end?: number, contentType?: string) => new Blob();
		this.bytes = async () => {
