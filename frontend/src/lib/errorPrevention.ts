...
export const PREVENTIVE_TIPS = {
	fileUpload: [
		...
		},
		{
			...
			condition: (file: File) => Boolean(file && file.size > 1024 * 1024)
		},
		{
			id: 'file-wrong-type',
	...
	email: [
		...
		},
		{
			...
			type: 'warning' as const,
			trigger: 'input' as const,
			condition: (value: string) => Boolean(value && value.length > 3 && !value.includes('@'))
		},
		{
			...
			type: 'warning' as const,
			trigger: 'input' as const,
			condition: (value: string) => Boolean(value && value.includes('@') && !value.includes('.'))
		}
	],
	...
	password: [
		...
		},
		{
			...
			type: 'warning' as const,
			trigger: 'input' as const,
			condition: (value: string) => Boolean(value && value.length > 0 && value.length < 8)
		},
		{
			id: 'password-weak',
			title: 'Weak Password',
	...
	filename: [
		...
		},
		{
			...
			type: 'warning' as const,
			trigger: 'input' as const,
			condition: (value: string) => Boolean(value && !VALIDATION_PATTERNS.filename.test(value))
		}
	]
};
